    ...
    protected ItemStackHandler createHandler() {
        return new ItemStackHandler(4){
            ...
            public boolean isItemValid(int slot, @NotNull ItemStack stack) {
                if(slot == 0){
                   return getAllRecipeInputsAsItems(ModRecipes.MILL_RECIPE_RECIPE_TYPE, world).contains(stack.getItem());
                }
                if(slot == 1 || slot == 2){
                    List<IRecipe<?>> recipes = findRecipeByType(ModRecipes.MILL_RECIPE_RECIPE_TYPE, world).stream().filter(iRecipe -> {
